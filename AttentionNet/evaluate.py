...
def evaluate(args, model=None, data_loader=None, sr=None):
    ...
    with ProcessPoolExecutor(args.num_workers) as pool:
        with torch.no_grad():
            ...
            for i, data in enumerate(iterator):
                ...
                #     sources, torch.stack(estimate, dim=1), lengths)
                sisnr_loss, snr, estimate, reorder_estimate, min_snr = cal_loss(
                    sources, torch.squeeze(estimate, 0), lengths)
                ...
                sdr = batch_SDR_torch(estimate, sources)
                sdrs.update(sdr)
    ...
    logger.info(
        bold(f'Test set performance: SISNRi={sisnr:.2f} PESQ={pesq}, STOI={stoi}, SDR={sdrs.avg.item()}.'))
    return sisnr, pesq, stoi, sdrs.avg.item()
...
def main():
    ...
    logger.debug(args)
    sisnr, pesq, stoi, sdr= evaluate(args)
    json.dump({'sisnr': sisnr,
               'pesq': pesq, 'stoi': stoi, 'sdr': sdr}, sys.stdout)
    sys.stdout.write('\n')
