...
from .models.swave_BiGRU import SWave
...
class Solver(object):
    ...
    def train(self):
        ...
        for epoch in range(len(self.history), self.epochs):
            ...
            metrics = {'epoch': epoch+1, 'train': train_loss,
            ...
            if (epoch + 1) % self.eval_every == 0 or epoch == self.epochs - 1:
                ...
                with swap_state(self.model, self.best_state):
                    sisnr, pesq, stoi ,sdr = evaluate(
                        self.args, self.model, self.tt_loader, self.args.sample_rate)
                metrics.update({'sisnr': sisnr, 'pesq': pesq, 'stoi': stoi, 'sdr': sdr})
        ...
        for i, data in enumerate(logprog):
            ...
            with torch.autograd.set_detect_anomaly(True):
                for c_idx, est_src in enumerate(estimate_source):
                    ...
                    loss_i = 0
                    # SI-SNR loss
                    sisnr_loss, snr, est_src, reorder_est_src, min_snr = cal_loss(
