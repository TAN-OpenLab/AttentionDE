...
def cal_loss(source, estimate_source, source_lengths):
    ...
    Args:
        ...
        source_lengths: [B]
    """
    max_snr, perms, max_snr_idx, snr_set, min_snr = cal_si_snr_with_pit(source,
                                                               estimate_source,
                                                               source_lengths)
    ...
    return loss, max_snr, estimate_source, reorder_estimate_source, min_snr
...
def cal_si_snr_with_pit(source, estimate_source, source_lengths):
    ...
    # Step 1. Zero-mean norm 经过处理的数据符合标准正态分布，即均值为0，标准差为1
    ...
    pair_wise_si_snr = torch.transpose(pair_wise_si_snr, 1, 2).contiguous()
    ...
    min_snr, _ = torch.min(snr_set, dim=1, keepdim=True)
    max_snr /= C
    return max_snr, perms, max_snr_idx, snr_set / C, min_snr
