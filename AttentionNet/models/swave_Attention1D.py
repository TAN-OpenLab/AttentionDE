...
class MulCatBlock(nn.Module):
    ...
    def forward(self, input):
        ...
        #Cat after parallel BiLSTMs
        gated_output = torch.cat([gated_output, output], 2)
        gated_output = self.block_projection(
            gated_output.contiguous().view(-1, gated_output.shape[2])).view(output.shape).contiguous()
        return gated_output
...
class Separator(nn.Module):
    ...
    def create_chuncks(self, input, segment_size):
        ...
        segments1 = input[:, :, :-segment_stride].contiguous().view(batch_size,
                                                                    dim, -1, segment_size).contiguous()
        segments2 = input[:, :, segment_stride:].contiguous().view(
            batch_size, dim, -1, segment_size).contiguous()
        segments = torch.cat([segments1, segments2], 3).view(
            batch_size, dim, -1, segment_size).transpose(2, 3).contiguous().contiguous()
...
class SWave(nn.Module):
    ...
    def __init__(self, N, L, H, R, C, sr, segment, input_normalize):
        ...
        self.segment_size = int(
            np.sqrt(2 * self.sr * self.segment / (self.L/2))) # chunk size
    ...
    def forward(self, mixture):
        ...
        for ii in range(len(output_all)):
            output_ii = output_all[ii].view(
                mixture.shape[0], self.C, self.N, mixture_w.shape[2]).contiguous()
...
class Encoder(nn.Module):
    def __init__(self, L, N):
        ...
        # self.acon = AconC(1, 128, self.conv.shape[2])
        self.strans = STrans(N)

    def forward(self, mixture):
        mixture = torch.unsqueeze(mixture, 1)
        # print('self.conv(mixture) shape', self.conv(mixture).shape)
        # mixture_w = self.acon(self.conv(mixture))
        mixture_new = F.relu(self.conv(mixture))
        x = self.conv(mixture_new)
        x = x.permute(0, 2, 1)
...
class Decoder(nn.Module):
    ...
    def forward(self, est_source):
        est_source = torch.transpose(est_source, 2, 3).contiguous()
