...
class MulCatBlock(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, dropout=0, bidirectional=False):
        ...
        # change LSTM to GRU
        self.rnn = nn.GRU(input_size, hidden_size, 1, dropout=dropout,
                          batch_first=True, bidirectional=bidirectional)
        self.rnn_proj = nn.Linear(hidden_size * self.num_direction, input_size)

        self.gate_rnn = nn.GRU(input_size, hidden_size, num_layers=1,
    ...
    def forward(self, input):
        ...
        # rnn_output = [128, 126, 256]

        rnn_output = self.rnn_proj(rnn_output.contiguous().view(-1, rnn_output.shape[2])).view(
            output.shape).contiguous()
        # rnn_output = [128, 126, 128]

        # run gate rnn module
        gate_rnn_output, _ = self.gate_rnn(output)
        gate_rnn_output = self.gate_rnn_proj(gate_rnn_output.contiguous().view(-1, gate_rnn_output.shape[2])).view(
            output.shape).contiguous()

        # apply gated rnn
        gated_output = torch.mul(rnn_output, gate_rnn_output)
        # gated_output = [128, 126, 128]
